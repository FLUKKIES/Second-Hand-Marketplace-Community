...
export default function LoginPage() {
    ...
    const [banDetails, setBanDetails] = useState<{ reason?: string; expiresAt?: string } | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Check for Ban Error (403)
            const data = err.response?.data;
            if (err.response?.status === 403 && (data?.message === 'Your account has been suspended' || data?.reason || data?.expiresAt)) {
                setBanDetails({
                    reason: data.reason,
            ...
            if (msg && (msg.includes("not found") || msg.includes("password"))) {
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <div>
                                <div className="relative">
                                    ...
                                    >
                                        {showPassword ? (
                                            <EyeOff className="h-5 w-5" /> // ถ้าเปิดอยู่ ให้โชว์รูปปิดตา
                                        ) : (
                                            <Eye className="h-5 w-5" />    // ถ้าปิดอยู่ ให้โชว์รูปเปิดตา
                                        )}
                                    </button>
                                </div>
                            </div>
                        ...
                        <div className="mt-6">
                            ...
                            <div className="mt-6">
                                <Button
                                    ...
                                    variant="outline"
                                    className="w-full flex justify-center items-center gap-3 py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all"
                                >
                                    {/* Google SVG Icon */}
