    ...
    try {
      ...
      if (keyword.length > 5 && !minRating && !type) {
        const interpretation =
          await this.ollamaService.interpretSearchQuery(keyword);
        if (interpretation) {
      ...
      if (groupId)
        postFilter = Prisma.sql`${postFilter} AND p."groupId" = ${groupId}`;
      if (categoryId)
        postFilter = Prisma.sql`${postFilter} AND g."categoryId" = ${categoryId}`;
      if (type && type !== 'ALL')
        postFilter = Prisma.sql`${postFilter} AND p."type"::text = ${type}`;

      // Base Filters for PRODUCTS
      let productFilter = Prisma.sql`p."deletedAt" IS NULL AND pd.embedding IS NOT NULL`;
      if (groupId)
        productFilter = Prisma.sql`${productFilter} AND p."groupId" = ${groupId}`;
      if (categoryId)
        productFilter = Prisma.sql`${productFilter} AND g."categoryId" = ${categoryId}`;
      if (type && type !== 'ALL')
        productFilter = Prisma.sql`${productFilter} AND p."type"::text = ${type}`;
      ...
      let mergedResults = Array.from(combinedMap.entries()).map(
      ...
      const postsWithScore = posts.map((post) => {
        const score =
          mergedResults.find((r) => r.id === post.id)?.similarity || 0;

        // Calculate simplistic avg rating for frontend
        const totalRating = post.author.reviewsReceived.reduce(
      ...
      if (sortBy === 'PRICE_ASC') {
        postsWithScore.sort((a, b) => {
          const priceA = a.products?.[0]?.price
            ? Number(a.products[0].price)
            : Infinity;
          const priceB = b.products?.[0]?.price
            ? Number(b.products[0].price)
            : Infinity;
