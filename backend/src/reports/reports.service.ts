...
export class ReportsService {
  ...
  async takeAction(reportId: string, dto: ReportActionDto) {
    ...
    if (report.targetType === TargetType.USER) {
      ...
      dto.action !== ReportAction.DISMISS
    ) {
  ...
  private async handleUserSanction(userId: string, dto: ReportActionDto) {
    switch (dto.action) {
      case ReportAction.WARN:
        await this.usersService.warnUser(
          userId,
          dto.warningMessage || 'You have received a warning.',
        );
        break;
      case ReportAction.TEMP_BAN:
        if (!dto.banDurationDays)
          throw new Error('Duration required for temp ban');
