...
export class PostsService {
  ...
  async create(userId: string, dto: CreatePostDto) {
    if (
      ...
      throw new BadRequestException(
        'Selling post must have at least one product',
      );
    }
    ...
    return await this.prisma.$transaction(
      async (tx) => {
        ...
        if (
          ...
          await Promise.all(
            dto.products.map(async (p, index) => {
              // 1. Create Product
  ...
  async findOne(id: string, userId?: string) {
    ...
    const lockedProducts = await this.prisma.product.findMany({
      where: {
        postId: id,
        OR: [{ orderItems: { some: {} } }, { offers: { some: {} } }],
      },
      select: { id: true },
    });
  ...
  async remove(userId: string, postId: string) {
    ...
    if (post.products && post.products.length > 0) {
      for (const product of post.products) {
        ...
        const acceptedOffers = product.offers.some(
          (o) => o.status === 'ACCEPTED',
        );
        if (acceptedOffers) {
          hasActiveTransaction = true;
    ...
    if (hasActiveTransaction) {
      ...
      return {
        message:
          'Post archived (Soft Deleted) because it has related transactions.',
      };
    } else {
  ...
  // 5. Update Post
  async update(userId: string, postId: string, dto: any) {
    // Using any for dto temporarily to avoid strict type issues with PartialType
    const post = await this.prisma.post.findUnique({
      where: { id: postId },
    ...
    if (dto.products && post.type === 'SELLING') {
      ...
      const incomingProductIds = new Set(
        incomingProducts.map((p) => p.id).filter((id) => id),
      );
      ...
      for (const id of deletedProductIds) {
        if (lockedProductIds.has(id)) {
          throw new BadRequestException(
            'Cannot delete product that already has an offer or order',
          );
        }
      }
