...
export class OffersService {
  ...
  async create(buyerId: string, dto: CreateOfferDto) {
    ...
    if (!buyerAddresses || buyerAddresses.length === 0) {
      throw new BadRequestException(
        'Please add a shipping address before making an offer',
      );
    }
    ...
    if (product.post.authorId === buyerId)
      throw new BadRequestException('Cannot offer your own product');
    if (product.stock <= 0)
  ...
  async respond(sellerId: string, offerId: string, dto: RespondOfferDto) {
    ...
    if (!offer) throw new NotFoundException('Offer not found');
    if (offer.product.post.authorId !== sellerId)
      throw new ForbiddenException('Not owner of this product');

    // Offer must be PENDING, or COUNTER_OFFERED and it's the seller's turn
    const isSellerTurn =
      offer.status === OfferStatus.PENDING ||
      (offer.status === OfferStatus.COUNTER_OFFERED &&
        offer.lastCounteredBy === CounteredBy.BUYER);
        ...
        if (updatedProduct.stock <= 0) {
          ...
          await tx.offer.updateMany({
            where: {
              ...
              id: { not: offerId },
              status: {
                in: [OfferStatus.PENDING, OfferStatus.COUNTER_OFFERED],
              },
            },
            data: {
              status: OfferStatus.REJECTED,
  ...
  async handleCron() {
    ...
    if (expiredCheckoutOffers.length > 0) {
      console.log(
        `[Cron] Expired ${expiredCheckoutOffers.length} accepted offers (Checkout Timeout)`,
      );
